...
CheckSet_ROILabelVars = function(data, path){
  if(interactive()){
    ...
    if(length(Lab.txt.levs) == 0){
      message("No labels used - continue.")
      return("No_Labels_Used")
    } else {
      ...
      if(file.exists(csvDir) == FALSE){
        ...
        message(paste("_________________________________________________________________\n\n",
                      ...
                      "   5) Rerun the Check_and_Set_Labelling_Variables function.\n\n",
        ...
        if(any(Label.csv.data$ROI_Label_code              == "<fill variable name here>" |
          ...
          # Check that labels from CSV match labels from the data imported from text files
          ...
          if(all(Lab.csv.chars == Lab.csv.chars) == FALSE){
            message(paste("\n________________________________________________________________\n\n",
                          "Error: characters used in ROI Labels do not match between the\n",
            ...
            message(paste("Suggestions:\n",
                          "First check the Labels are correct in the csv template.\n",
          ...
          if(all(Lab.csv.chars == Lab.csv.chars) == TRUE){
            ...
            if(x == 1){
              ...
              Label.Translator = data.frame(ROI.Label = levels(as.factor(data$ROI.Label)))
              ...
              # For each row in Label Translator calculate which Label variables have a TRUE or FALSE
              # This is based on checking whether the string is contained or not
              # Note: if one variable label is "b" and a second variable label is "bb"
              ...
              for(i in 1:nrow(Label.Translator)){
                Label.Translator[i,2:(N.Lab.txt.chars+1)] = rbind(lapply(Label.csv.data$ROI_Label_code,
                                                                         function(x){ grepl(x, Label.Translator$ROI.Label[i], fixed = TRUE)}))
              }
              ...
              Label.Translator[fact.cols] <- lapply(Label.Translator[fact.cols],
                                                    factor)
              Label.Translator$ROI.Label = addNA(Label.Translator$ROI.Label)
